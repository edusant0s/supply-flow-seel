...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .freight-cargo-items-table {
      width: 100%;
      min-width: 920px;
      border-collapse: collapse;
    }

    .freight-cargo-items-table th,
    .freight-cargo-items-table td {
      padding: 8px;
    ...
    .freight-cargo-items-table td:first-child,
    .freight-cargo-items-table th:first-child {
      width: 44px;
      text-align: center;
      color: #64748b;
      font-weight: 950;
    ...
    .freight-cargo-items-table input {
      height: 40px;
      padding: 9px 10px;
      border-radius: 10px;
      font-size: 13px;
    ...
    .freight-cargo-items-summary strong {
      color: var(--seel-blue-dark);
      font-weight: 950;
    }

    .freight-conditional-hidden {
      display: none !important;
    }

    .freight-return-doc-message {
      grid-column: 1 / -1;
    ...
    @media (max-width: 720px) {
      ...
      .freight-cargo-items-head button {
        width: 100%;
      }
      .freight-cargo-items-panel {
        padding: 12px;
    ...
    .form-editor-main-grid{display:grid;grid-template-columns:1.4fr .75fr 1.2fr;gap:9px}.form-editor-main-grid.section-grid{grid-template-columns:1fr}.form-editor-main-grid.cargo-label-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
    ...
    .form-editor-item-actions{display:grid;grid-template-columns:repeat(2,34px);gap:6px}.form-editor-item-actions button{width:34px;height:34px;min-height:34px;padding:0;border-radius:10px;background:#f1f5f9;color:#0e4a6e;border:1px solid rgba(100,116,139,.18);font-size:14px}.form-editor-item-actions .delete-editor-item{grid-column:1/-1;width:74px;color:#991b1b;background:#fff1f2}.cargo-labels-item .form-editor-item-actions .delete-editor-item{width:96px;font-size:11px}
    ...
    #kanbanBoard .freight-card:focus-visible {
      transform: translateY(-1px) !important;
      box-shadow: 0 10px 22px rgba(10,46,61,.16) !important;
      outline: none !important;
    }

    #kanbanBoard .freight-card.red { border-left-color: #e7000b !important; }
    #kanbanBoard .freight-card.yellow { border-left-color: #d97706 !important; }
    #kanbanBoard .freight-card.green { border-left-color: #059669 !important; }
    ...
    #kanbanBoard .freight-priority-bubble.urgent { background: rgba(217,119,6,.14) !important; color: #d97706 !important; }
    #kanbanBoard .freight-priority-bubble.normal { background: rgba(27,109,142,.12) !important; color: #1b6d8e !important; }
    ...
    #kanbanBoard .freight-compact-lines > div > span:last-child {
      overflow: hidden !important;
      text-overflow: ellipsis !important;
      white-space: nowrap !important;
    }
    ...
    #kanbanBoard .freight-mini-badge.urgent { background: rgba(217,119,6,.12) !important; color: #d97706 !important; }
    #kanbanBoard .freight-mini-badge.normal { background: rgba(27,109,142,.1) !important; color: #1b6d8e !important; }
    #kanbanBoard .freight-mini-badge.traffic-red { background: rgba(231,0,11,.1) !important; color: #e7000b !important; }
    #kanbanBoard .freight-mini-badge.traffic-yellow { background: rgba(217,119,6,.12) !important; color: #d97706 !important; }
    #kanbanBoard .freight-mini-badge.traffic-green { background: rgba(5,150,105,.1) !important; color: #059669 !important; }
    #kanbanBoard .freight-mini-badge.traffic-gray { background: #e2e8f0 !important; color: #475569 !important; }
    #kanbanBoard .freight-mini-badge.phase { background: rgba(27,109,142,.1) !important; color: #1b6d8e !important; }
    ...
    #kanbanBoard .freight-compact-meta strong { color: var(--text) !important; }
    #kanbanBoard .meta-status { font-weight: 900 !important; white-space: nowrap !important; }
    #kanbanBoard .meta-status.red { color: #e7000b !important; }
    #kanbanBoard .meta-status.yellow { color: #d97706 !important; }
    #kanbanBoard .meta-status.green { color: #059669 !important; }
    ...
    #kanbanBoard .freight-compact-timer > div > span:first-child {
      overflow: hidden !important;
      text-overflow: ellipsis !important;
      white-space: nowrap !important;
    }
    ...
    #kanbanBoard .freight-compact-actions button:last-child {
      background: var(--card-solid) !important;
      color: #1b6d8e !important;
      border-color: rgba(27,109,142,.4) !important;
    }
    ...
    #kanbanBoard .freight-compact-actions .btn-email {
      ...
      min-height: 29px !important;
      padding: 5px 8px !important;
      border: 1px solid rgba(15,118,110,.4) !important;
      border-radius: 8px !important;
      background: var(--card-solid) !important;
    ...
    #kanbanBoard .freight-compact-actions [data-action="delete"] {
      grid-column: 1 / -1 !important;
      background: var(--card-solid) !important;
      color: #e7000b !important;
      border-color: rgba(231,0,11,.4) !important;
    }

    #kanbanBoard .freight-compact-actions [data-action="delete"]:hover {
      background: rgba(231,0,11,.08) !important;
      border-color: rgba(231,0,11,.55) !important;
    }
...
<style id="freight-responsible-note-editor-style">
  ...
  <style>
    ...
    #kanbanBoard .freight-line-icon {
      ...
      background: rgba(27,109,142,.1) !important;
      color: #1b6d8e !important;
    }
...
<main>
    ...
    <section id="form" class="section active">
      <div class="panel">
        <h2 id="freightFormTitle" data-cargo-label-key="formTitle">Nova Solicitação de Frete</h2><p id="freightFormSubtitle" class="muted" data-cargo-label-key="formSubtitle">Preencha os dados da RM, endereços, carga, documentação e fornecedor. Ao criar, o card entra no Kanban.</p>
        ...
        <form id="freightForm">
          <div class="form-grid">
            ...
            <div><label>Tipo de movimentação *</label><select id="tipoMovimentacao" required><option value="">Selecione</option><option>Movimentação entre obras</option><option>Coleta em fornecedor</option><option>Coleta no fornecedor</option><option>Entrega em obra</option><option>Devolução</option><option>Devolução ao fornecedor</option><option>Transferência interna</option><option>Outro</option></select></div>
            ...
            <div><label>Requer nota fiscal?</label><select id="notaFiscal"><option value="">Selecione</option><option>SIM. NOTA FISCAL DE SIMPLES REMESSA</option><option>SIM. NOTA FISCAL DE DEVOLUÇÃO - SETOR FISCAL</option><option>NÃO. O FORNECEDOR IRÁ EMITIR</option><option>NÃO PRECISA</option></select></div>
            ...
            <div><label>Nome / descriÃ§Ã£o da carga</label><input id="nomeCarga" placeholder="Ex.: Uniformes, tubos, equipamentos" /></div>
            <div><label>Valor unitÃ¡rio da carga R$</label><input id="valorUnitarioCarga" type="number" min="0" step="0.01" placeholder="Ex.: 125.90" /></div>
          ...
          <div class="freight-cargo-items-panel" id="freightCargoItemsPanel">
            <div class="freight-cargo-items-head">
              <div>
                <h3 id="freightCargoItemsTitle" data-cargo-label-key="title">Itens da carga para Nota Fiscal</h3>
                <p id="freightCargoItemsSubtitle" data-cargo-label-key="subtitle">Informe cada item separadamente para que a NF de simples remessa seja criada com os produtos corretos.</p>
              </div>
              <button id="addFreightCargoItem" class="btn-secondary btn-small" type="button" data-cargo-label-key="addButton">Adicionar item</button>
            </div>
            <div class="freight-cargo-items-scroll">
              <table class="freight-cargo-items-table">
                <thead>
                  <tr>
                    <th>#</th>
                    <th style="min-width:280px" data-cargo-label-key="description">Nome / descrição</th>
                    <th style="width:130px" data-cargo-label-key="quantity">Quantidade</th>
                    <th style="width:150px" data-cargo-label-key="unitValue">Valor unitário</th>
                    <th style="min-width:210px" data-cargo-label-key="dimensions">Dimensões</th>
                    <th style="width:140px;text-align:right">Total</th>
                    <th style="width:58px"></th>
                  </tr>
                </thead>
                <tbody id="freightCargoItemsBody"></tbody>
              </table>
            </div>
            <div class="freight-cargo-items-summary">
              <span>Itens: <strong id="freightCargoItemsCount">0</strong></span>
              <span>Total: <strong id="freightCargoItemsTotal">R$ 0,00</strong></span>
            </div>
          </div>
  ...
  <script>
    ...
    const FREIGHT_SUPPLIER_RETURN_MOVEMENT = "Devolução ao fornecedor";
    const FREIGHT_SUPPLIER_PICKUP_MOVEMENT = "Coleta no fornecedor";
    const FREIGHT_RETURN_DOCUMENTATION_VALUE = "SIM. NOTA FISCAL DE DEVOLUÇÃO - SETOR FISCAL";
    const FREIGHT_RETURN_DOCUMENTATION_MESSAGE = "Solicitar Nota Fiscal de Devolução ao Setor Fiscal.";
    const FREIGHT_DOCUMENTATION_DIVERGENCE_MESSAGE = "A Nota Fiscal de Devolução só se aplica ao tipo de movimentação 'Devolução ao fornecedor'. Ajuste o tipo de movimentação ou a documentação selecionada.";
    const FREIGHT_DOCUMENTATION_OPTIONS = [
      { value: "", text: "Selecione" },
      { value: "SIM. NOTA FISCAL DE SIMPLES REMESSA", text: "SIM. NOTA FISCAL DE SIMPLES REMESSA" },
      { value: FREIGHT_RETURN_DOCUMENTATION_VALUE, text: FREIGHT_RETURN_DOCUMENTATION_VALUE },
      { value: "NÃO. O FORNECEDOR IRÁ EMITIR", text: "NÃO. O FORNECEDOR IRÁ EMITIR" },
      { value: "NÃO PRECISA", text: "NÃO PRECISA" }
    ];
    const STORAGE_KEY = "gestao_fretes_solicitacoes_v1";
    const FORM_EDITOR_STORAGE_KEY = "gestao_fretes_formulario_config_v1";
    const FREIGHT_CARGO_LABELS_STORAGE_KEY = "gestao_fretes_cargo_items_labels_v1";
    const DEFAULT_FREIGHT_CARGO_LABELS = {
      formTitle: "Nova Solicitação de Frete",
      formSubtitle: "Preencha os dados da RM, endereços, carga, documentação e fornecedor. Ao criar, o card entra no Kanban.",
      title: "Itens da carga para Nota Fiscal",
      subtitle: "Informe cada item separadamente para que a NF de simples remessa seja criada com os produtos corretos.",
    ...
    let freightCargoLabels = loadFreightCargoLabels();
    ...
    */
    ...
    setupLoggedRequesterFromHost(true);
    setupSupplierReturnLogic(false);
    document.getElementById("addFreightCargoItem")?.addEventListener("click", () => addFreightCargoItemRow());
    renderFreightCargoItemRows();

    (function loadMarketplaceDeliveryDraft() {
      try {
        const raw = sessionStorage.getItem("supply-flow:fretes-draft");
        if (!raw) return;
        sessionStorage.removeItem("supply-flow:fretes-draft");
        const draft = JSON.parse(raw);
        if (!draft || draft.targetModule !== "fretes") return;
        ...
        setValue("solicitante", draft.requester?.name);
        setValue("telefoneSolicitante", draft.requester?.phone);
        setValue("emailSolicitante", draft.requester?.email);
        setValue("tipoMovimentacao", "Entrega em obra");
        setValue(
          "particularidadeObra",
          ["Origem: Marketplace (Estoque de obras).", draft.notes || ""].filter(Boolean).join(" ")
        );

        const modoColeta = document.getElementById("modoLocalColeta");
        if (modoColeta) {
          modoColeta.value = "LINK_MAPS";
          modoColeta.dispatchEvent(new Event("change"));
        }
        setValue("linkGoogleMapsColeta", mapsSearchLink(draft.origin));

        const modoEntrega = document.getElementById("modoLocalEntrega");
        if (modoEntrega) {
          modoEntrega.value = "LINK_MAPS";
          modoEntrega.dispatchEvent(new Event("change"));
        }
        setValue("linkGoogleMapsEntrega", mapsSearchLink(draft.destination));

        (draft.materials || []).forEach((material) => {
          if (typeof addFreightCargoItemRow === "function") {
            addFreightCargoItemRow({ description: material.description, quantity: material.quantity });
          }
        });

        window.alert(
          "Dados do pedido de entrega do Marketplace (Estoque de obras) foram carregados no formulário. Complete o número da RM, número da obra, tipo de veículo e o valor unitário da carga antes de enviar."
        );
      } catch (err) {
        console.error("Erro ao carregar rascunho de entrega do Marketplace:", err);
      }
    })();
    ...
    function initializeFreightFormEditor() {
      ...
      ensureCargoFiscalFieldsInSchema();
      ensureSupplierReturnMovementOptionInSchema();
      ensureFreightDocumentationFieldInSchema();
      renderFreightFormFromSchema(false);
      renderFreightCargoLabels();
    }

    function ensureSupplierReturnMovementOptionInSchema() {
      const field = freightFormSchema.find(item => item.kind === "field" && item.id === "tipoMovimentacao");
      if (!field) return;
      field.type = "select";
      field.required = true;
      const options = normalizedEditorOptions(field);
      const nextOptions = [...options];
      const requiredOptions = [
        { value: FREIGHT_SUPPLIER_RETURN_MOVEMENT, text: FREIGHT_SUPPLIER_RETURN_MOVEMENT },
        { value: FREIGHT_SUPPLIER_PICKUP_MOVEMENT, text: FREIGHT_SUPPLIER_PICKUP_MOVEMENT }
      ];
      let changed = false;
      requiredOptions.forEach(requiredOption => {
        const exists = nextOptions.some(option => normalizeFreightIntegrationText(option.value || option.text) === normalizeFreightIntegrationText(requiredOption.value));
        if (!exists) {
          nextOptions.push({ value: requiredOption.value, text: requiredOption.text });
          changed = true;
        }
      });
      if (changed) {
    ...
    function ensureFreightDocumentationFieldInSchema() {
      const field = freightFormSchema.find(item => item.kind === "field" && item.id === "notaFiscal");
      if (!field) return;
      field.type = "select";
      field.placeholder = "";
      field.required = false;
      const currentOptions = normalizedEditorOptions(field);
      const nextOptions = currentOptions.length ? [...currentOptions] : [];
      let changed = !currentOptions.length;
      FREIGHT_DOCUMENTATION_OPTIONS.forEach(defaultOption => {
        const exists = nextOptions.some(option => normalizeFreightIntegrationText(option.value || option.text) === normalizeFreightIntegrationText(defaultOption.value));
        if (!exists) {
          nextOptions.push(deepClone(defaultOption));
          changed = true;
        }
      });
      if (changed) {
    ...
    function ensureCargoFiscalFieldsInSchema() {
      const templates = [
        {
          key: "field:nomeCarga", kind: "field", id: "nomeCarga", label: "Nome / descriÃ§Ã£o da carga",
          type: "text", placeholder: "Ex.: Uniformes, tubos, equipamentos", required: false, full: false, visible: true,
          options: [], min: "", step: "", custom: false
        },
        {
          key: "field:valorUnitarioCarga", kind: "field", id: "valorUnitarioCarga", label: "Valor unitÃ¡rio da carga R$",
          type: "number", placeholder: "Ex.: 125.90", required: false, full: false, visible: true,
          options: [], min: "0", step: "0.01", custom: false
        }
      ];
      const existingIds = new Set(freightFormSchema.filter(item => item.kind === "field").map(item => item.id));
      const missing = templates.filter(item => !existingIds.has(item.id));
      if (!missing.length) return;
      let insertIndex = freightFormSchema.findIndex(item => item.kind === "field" && item.id === "totalPecas");
      if (insertIndex < 0) insertIndex = freightFormSchema.findIndex(item => item.kind === "field" && item.id === "materiais");
      freightFormSchema.splice(insertIndex >= 0 ? insertIndex : freightFormSchema.length, 0, ...missing.map(deepClone));
      saveFreightFormSchema(false);
    }
    ...
    function isSupplierReturnMovement(value) {
      const normalized = normalizeFreightIntegrationText(value);
      return normalized === "devolucao ao fornecedor" || (normalized.includes("devolucao") && normalized.includes("fornecedor"));
    }

    function isSupplierPickupMovement(value) {
      const normalized = normalizeFreightIntegrationText(value);
      return normalized.includes("coleta") && normalized.includes("fornecedor");
    }

    function requiresSupplierFieldsForMovement(value) {
      return isSupplierReturnMovement(value) || isSupplierPickupMovement(value);
    }

    function isReturnDocumentationValue(value) {
      const normalized = normalizeFreightIntegrationText(value);
      return normalized.includes("nota fiscal") && normalized.includes("devolucao") && normalized.includes("setor fiscal");
    }

    function getFreightFieldWrapper(fieldId) {
      const control = document.getElementById(fieldId);
      return control?.closest("[data-form-key]") || control?.parentElement || null;
    }

    function setFreightConditionalFieldState(fieldId, visible, required = false, clearWhenHidden = true) {
      const control = document.getElementById(fieldId);
      if (!control) return;
      const wrapper = getFreightFieldWrapper(fieldId);
      if (wrapper) wrapper.classList.toggle("freight-conditional-hidden", !visible);
      control.disabled = !visible;
      control.required = Boolean(visible && required);
      if (!visible && clearWhenHidden) control.value = "";
    }

    function ensureFreightInlineMessage(id, afterWrapper, className, text) {
      ...
      if (!message) {
        ...
        afterWrapper?.insertAdjacentElement("afterend", message);
      }
      return message;
    }

    function updateSupplierReturnFormState(preserveValues = true) {
      const movementControl = document.getElementById("tipoMovimentacao");
      const documentationControl = document.getElementById("notaFiscal");
      const documentationWrapper = getFreightFieldWrapper("notaFiscal");
      const isSupplierReturn = isSupplierReturnMovement(movementControl?.value);
      const requiresSupplierFields = requiresSupplierFieldsForMovement(movementControl?.value);

      setFreightConditionalFieldState("fornecedorNome", requiresSupplierFields, requiresSupplierFields, !preserveValues);
      setFreightConditionalFieldState("fornecedorCnpj", requiresSupplierFields, requiresSupplierFields, !preserveValues);

      if (!documentationControl || !documentationWrapper) return;

      const returnMessage = ensureFreightInlineMessage(
        "freightReturnDocumentationMessage",
        documentationWrapper,
        "freight-return-doc-message",
        FREIGHT_RETURN_DOCUMENTATION_MESSAGE
      );
      const divergenceWarning = ensureFreightInlineMessage(
        "freightDocumentationDivergenceWarning",
        returnMessage,
        "freight-form-warning",
        FREIGHT_DOCUMENTATION_DIVERGENCE_MESSAGE
      );

      if (isSupplierReturn) {
        documentationControl.value = FREIGHT_RETURN_DOCUMENTATION_VALUE;
        documentationControl.disabled = true;
        documentationControl.required = false;
        documentationWrapper.classList.add("freight-conditional-hidden");
        returnMessage.hidden = false;
        divergenceWarning.hidden = true;
        return;
      }

      documentationControl.disabled = false;
      documentationWrapper.classList.remove("freight-conditional-hidden");
      returnMessage.hidden = true;
      divergenceWarning.hidden = !isReturnDocumentationValue(documentationControl.value);
    }

    function setupSupplierReturnLogic(preserveValues = true) {
      const movementControl = document.getElementById("tipoMovimentacao");
      const documentationControl = document.getElementById("notaFiscal");
      if (movementControl && movementControl.dataset.supplierReturnBound !== "1") {
        movementControl.dataset.supplierReturnBound = "1";
        movementControl.addEventListener("change", () => updateSupplierReturnFormState(false));
      }
      if (documentationControl && documentationControl.dataset.supplierReturnBound !== "1") {
        documentationControl.dataset.supplierReturnBound = "1";
        documentationControl.addEventListener("change", () => updateSupplierReturnFormState(true));
      }
      updateSupplierReturnFormState(preserveValues);
    }
    ...
    function loadFreightCargoLabels() {
      try {
        const stored = JSON.parse(localStorage.getItem(FREIGHT_CARGO_LABELS_STORAGE_KEY) || "{}");
        return { ...DEFAULT_FREIGHT_CARGO_LABELS, ...(stored && typeof stored === "object" ? stored : {}) };
      } catch (error) {
        return { ...DEFAULT_FREIGHT_CARGO_LABELS };
      }
    }

    function saveFreightCargoLabels(showStatus = true) {
      localStorage.setItem(FREIGHT_CARGO_LABELS_STORAGE_KEY, JSON.stringify(freightCargoLabels));
      renderFreightCargoLabels();
      if (showStatus) updateFormEditorStatus("Rótulos dos itens salvos");
    }

    function freightCargoLabelText(key) {
      return Object.prototype.hasOwnProperty.call(freightCargoLabels, key)
        ? String(freightCargoLabels[key] ?? "")
        : String(DEFAULT_FREIGHT_CARGO_LABELS[key] ?? "");
    }

    function renderFreightCargoLabels() {
      document.querySelectorAll("[data-cargo-label-key]").forEach(element => {
        const key = element.dataset.cargoLabelKey;
        element.textContent = freightCargoLabelText(key);
      });
    }

    function renderFreightCargoLabelsEditorHtml() {
      const label = key => escapeHtml(freightCargoLabelText(key));
      return \`<article class="form-editor-item cargo-labels-item" data-cargo-label-editor="1">
        <div class="form-editor-order">NF</div>
        <div class="form-editor-content">
          <div class="form-editor-main-grid cargo-label-grid">
            <label>TÍTULO DO FORMULÁRIO<input data-cargo-label-prop="formTitle" value="\${label("formTitle")}"></label>
            <label class="full">SUBTÍTULO DO FORMULÁRIO<textarea data-cargo-label-prop="formSubtitle">\${label("formSubtitle")}</textarea></label>
            <label>TÍTULO DOS ITENS<input data-cargo-label-prop="title" value="\${label("title")}"></label>
            <label>TEXTO DO BOTÃO<input data-cargo-label-prop="addButton" value="\${label("addButton")}"></label>
            <label class="full">SUBTÍTULO<textarea data-cargo-label-prop="subtitle">\${label("subtitle")}</textarea></label>
            <label>CABEÇALHO DESCRIÇÃO<input data-cargo-label-prop="description" value="\${label("description")}"></label>
            <label>CABEÇALHO QUANTIDADE<input data-cargo-label-prop="quantity" value="\${label("quantity")}"></label>
            <label>CABEÇALHO VALOR UNITÁRIO<input data-cargo-label-prop="unitValue" value="\${label("unitValue")}"></label>
            <label>CABEÇALHO DIMENSÕES<input data-cargo-label-prop="dimensions" value="\${label("dimensions")}"></label>
          </div>
          <div class="form-editor-checks"><span style="font-size:10px;color:#64748b">Configuração da seção de itens da carga usada para gerar Nota Fiscal.</span></div>
        </div>
        <div class="form-editor-item-actions">
          <button class="delete-editor-item" type="button" title="Restaurar rótulos padrão" onclick="restoreFreightCargoLabels()">Restaurar</button>
        </div>
      </article>\`;
    }

    function bindFreightCargoLabelsEditor() {
      const list = document.getElementById("freightFormEditorList");
      if (!list) return;
      list.querySelectorAll("[data-cargo-label-prop]").forEach(element => {
        element.addEventListener(element.tagName === "TEXTAREA" ? "input" : "input", () => {
          const prop = element.dataset.cargoLabelProp;
          freightCargoLabels[prop] = element.value;
          saveFreightCargoLabels();
        });
      });
    }

    function restoreFreightCargoLabels() {
      if (!confirm("Restaurar os rótulos padrão dos itens da carga?")) return;
      freightCargoLabels = { ...DEFAULT_FREIGHT_CARGO_LABELS };
      saveFreightCargoLabels();
    ...
    function renderFreightFormFromSchema(preserveValues = true) {
      ...
      setupSupplierReturnLogic(preserveValues);
    }
    ...
    function renderFreightFormEditor() {
      ...
      if (!freightFormSchema.length) {
        list.innerHTML = renderFreightCargoLabelsEditorHtml() + '<div class="form-editor-empty">Nenhuma seção ou campo configurado.</div>';
        bindFreightCargoLabelsEditor();
        return;
      }
      list.innerHTML = renderFreightCargoLabelsEditorHtml() + freightFormSchema.map((item, index) => {
    ...
    function normalizeLoadedFreight(freight) {
      ...
      normalized.nomeCarga = normalized.nomeCarga || normalized.formData?.nomeCarga?.value || "";
      normalized.valorUnitarioCarga = normalized.valorUnitarioCarga || normalized.formData?.valorUnitarioCarga?.value || "";
      normalized.itensCarga = normalizeFreightCargoItems(normalized.itensCarga || normalized.cargoItems || normalized.itemsCarga || []);
      normalized.cargoItems = normalized.itensCarga;
    ...
    const NOTA_FISCAL_STORAGE_KEY = (() => {
      try {
        return String(window.SUPPLY_FLOW_CONTEXT?.sync?.notaFiscalStorageKey || "seel_nf_simples_remessa_v1");
      } catch (error) {
        return "seel_nf_simples_remessa_v1";
      }
    })();
    const NF_INITIAL_PHASE = "N\\u00e3o Iniciado";
    const NF_PROGRESS_PHASE = "Em Tratativa";
    const NF_DONE_PHASE = "Finalizado";

    function normalizeFreightIntegrationText(value) {
      return String(value || "")
        .normalize("NFD")
        .replace(/[\\u0300-\\u036f]/g, "")
        .toLowerCase()
    ...
    function freightRequiresNotaFiscal(value) {
      const normalized = normalizeFreightIntegrationText(value);
      if (!normalized) return false;
      if (normalized.startsWith("nao") || normalized.startsWith("sem ") || normalized.includes("nao precisa") || normalized.includes("sem nf") || normalized.includes("sem nota") || normalized.includes("fornecedor ira emitir")) return false;
      return normalized.includes("sim") || normalized.includes("simples remessa") || normalized.includes("precisa") || normalized.includes("preciso") || normalized.includes("necessita") || normalized.includes("emissao") || normalized === "nf";
    }

    function loadNotaFiscalRequests() {
      try {
        const stored = JSON.parse(localStorage.getItem(NOTA_FISCAL_STORAGE_KEY) || "[]");
    ...
    function saveNotaFiscalRequests(rows) {
      localStorage.setItem(NOTA_FISCAL_STORAGE_KEY, JSON.stringify(Array.isArray(rows) ? rows : []));
    }

    function nextNotaFiscalCode(rows) {
      const date = new Date();
      const prefix = \`NF-\${String(date.getMonth() + 1).padStart(2, "0")}\${String(date.getFullYear()).slice(-2)}-\`;
      const numbers = (Array.isArray(rows) ? rows : [])
        .filter(row => String(row?.code || "").startsWith(prefix))
        .map(row => Number(String(row.code).split("-").pop()) || 0);
      return prefix + String(Math.max(0, ...numbers) + 1).padStart(3, "0");
    }

    function linkedNotaFiscalIdForFreight(freight) {
      return String(freight?.notaFiscalSolicitacaoId || freight?.nfSimplesRemessaId || \`NF-FRETE-\${freight?.id || generateId()}\`).trim();
    }

    function normalizeFreightCnpj(value) {
    ...
    function legacyFreightDimensionsText(item) {
      const length = Math.max(0, parseFreightDecimal(item?.length ?? item?.comprimento ?? item?.comprimentoItem, 0));
      const width = Math.max(0, parseFreightDecimal(item?.width ?? item?.largura ?? item?.larguraItem, 0));
      const height = Math.max(0, parseFreightDecimal(item?.height ?? item?.altura ?? item?.alturaItem, 0));
      if (!(length || width || height)) return "";
      return \`C \${length || "-"} x L \${width || "-"} x A \${height || "-"} cm\`;
    }

    function normalizeFreightCargoItems(items) {
      return (Array.isArray(items) ? items : [])
        .map((item, index) => {
          const description = String(item?.description || item?.nome || item?.nomeCarga || item?.descricao || "").trim();
          const quantity = Math.max(0, parseFreightDecimal(item?.quantity ?? item?.quantidade ?? item?.qtd, 0));
          const unitValue = Math.max(0, parseFreightDecimal(item?.unitValue ?? item?.valorUnitario ?? item?.valor_unitario, 0));
          const length = Math.max(0, parseFreightDecimal(item?.length ?? item?.comprimento ?? item?.comprimentoItem, 0));
          const width = Math.max(0, parseFreightDecimal(item?.width ?? item?.largura ?? item?.larguraItem, 0));
          const height = Math.max(0, parseFreightDecimal(item?.height ?? item?.altura ?? item?.alturaItem, 0));
          const dimensions = String(item?.dimensions || item?.dimensoes || item?.dimensoesItem || legacyFreightDimensionsText(item)).trim();
          const ncm = String(item?.ncm || "").replace(/\\D/g, "").slice(0, 8);
          const id = String(item?.id || \`ITEM-\${index + 1}\`);
          return { id, description, quantity, unitValue, dimensions, length, width, height, ncm };
        })
        .filter(item => item.description || item.quantity > 0 || item.unitValue > 0 || item.dimensions || item.length > 0 || item.width > 0 || item.height > 0 || item.ncm);
    }

    function freightCargoFallbackItems(freight) {
      const quantity = Math.max(1, numericFromFreightText(freight.totalPecas || freight.volumes, 1));
      const totalValue = Math.max(0, parseFreightDecimal(freight.valorTotalCarga, 0));
      const explicitUnitValue = Math.max(0, parseFreightDecimal(freight.valorUnitarioCarga, 0));
      const unitValue = explicitUnitValue > 0 ? explicitUnitValue : totalValue > 0 ? totalValue / quantity : 0;
      const descriptionParts = [
        freight.nomeCarga || freight.materiais || freight.tipoMaterial,
        freight.nomeCarga && freight.materiais ? freight.materiais : "",
        freight.dimensoesCarga,
        freight.pesoTotalCarga ? \`Peso total: \${freight.pesoTotalCarga} kg\` : "",
        freight.totalPecas ? \`Volumes: \${freight.totalPecas}\` : "",
        freight.numeroRM ? \`RM: \${freight.numeroRM}\` : ""
      ].filter(Boolean);
      return [{
        id: \`P-\${freight.id || Date.now()}\`,
        description: descriptionParts.join(" | ") || "Carga vinculada a solicitacao de frete",
        quantity,
        unitValue,
        dimensions: freight.dimensoesCarga || "",
        length: 0,
        width: 0,
        height: 0,
        ncm: ""
    ...
    function freightCargoItemsFromFreight(freight) {
      const normalized = normalizeFreightCargoItems(freight?.itensCarga || freight?.cargoItems || freight?.itemsCarga || freight?.freightIntegration?.itensCarga);
      return normalized.length ? normalized : freightCargoFallbackItems(freight || {});
    }

    function freightCargoItemsForDisplay(freight) {
      const normalized = normalizeFreightCargoItems(freight?.itensCarga || freight?.cargoItems || freight?.itemsCarga || freight?.freightIntegration?.itensCarga);
      if (normalized.length) return normalized;
      const hasLegacyCargoInfo = Boolean(freight?.nomeCarga || freight?.materiais || freight?.tipoMaterial || freight?.valorUnitarioCarga || freight?.valorTotalCarga || freight?.totalPecas || freight?.volumes);
      return hasLegacyCargoInfo ? freightCargoFallbackItems(freight || {}) : [];
    }

    function formatFreightCargoItemDimensions(item) {
      const dimensions = String(item?.dimensions || item?.dimensoes || item?.dimensoesItem || "").trim();
      if (dimensions) return \`Dim.: \${dimensions}\`;
      const length = Number(item?.length) || 0;
      const width = Number(item?.width) || 0;
      const height = Number(item?.height) || 0;
      if (!(length || width || height)) return "";
      return \`Dim.: \${length || "-"} x \${width || "-"} x \${height || "-"} cm\`;
    }

    function formatFreightCargoItemsText(items) {
      const normalized = normalizeFreightCargoItems(items);
      if (!normalized.length) return "-";
      return normalized.map((item, index) => {
        const total = (Number(item.quantity) || 0) * (Number(item.unitValue) || 0);
        const dimensions = formatFreightCargoItemDimensions(item);
        const parts = [
          \`\${index + 1}. \${item.description || "Item sem descriÃ§Ã£o"}\`,
          \`Qtd: \${item.quantity || 0}\`,
          \`Unit.: \${formatCurrency(item.unitValue)}\`,
    ...
    function renderFreightCargoItemRows(items = []) {
      const body = document.getElementById("freightCargoItemsBody");
      if (!body) return;
      body.innerHTML = "";
      const rows = normalizeFreightCargoItems(items);
      if (rows.length) rows.forEach(item => addFreightCargoItemRow(item));
      else addFreightCargoItemRow();
      updateFreightCargoItemsSummary();
    }

    function addFreightCargoItemRow(data = {}) {
      const body = document.getElementById("freightCargoItemsBody");
      if (!body) return;
      const item = normalizeFreightCargoItems([data])[0] || {};
      const tr = document.createElement("tr");
      tr.dataset.itemId = item.id || generateId();
      tr.innerHTML = \`
        <td class="freight-cargo-item-number"></td>
        <td><input class="freight-cargo-desc" placeholder="Ex.: Uniforme brim azul" value="\${escapeHtml(item.description || "")}" /></td>
        <td><input class="freight-cargo-qty" type="number" min="0" step="0.0001" placeholder="0" value="\${item.quantity || ""}" /></td>
        <td><input class="freight-cargo-unit" type="number" min="0" step="0.01" placeholder="0,00" value="\${item.unitValue || ""}" /></td>
        <td><input class="freight-cargo-dimensions" placeholder="Ex.: 120 x 80 x 60 cm" value="\${escapeHtml(item.dimensions || legacyFreightDimensionsText(item))}" /></td>
        <td class="freight-cargo-item-total">R$ 0,00</td>
        <td><button type="button" class="btn-danger btn-small freight-cargo-remove" title="Excluir item">x</button></td>
      \`;
      body.appendChild(tr);
      tr.querySelectorAll("input").forEach(input => input.addEventListener("input", updateFreightCargoItemsSummary));
      tr.querySelector(".freight-cargo-remove")?.addEventListener("click", () => {
    ...
    function updateFreightCargoItemsSummary() {
      const rows = Array.from(document.querySelectorAll("#freightCargoItemsBody tr"));
      let total = 0;
      let filled = 0;
      rows.forEach((tr, index) => {
        const quantity = parseFreightDecimal(tr.querySelector(".freight-cargo-qty")?.value, 0);
        const unitValue = parseFreightDecimal(tr.querySelector(".freight-cargo-unit")?.value, 0);
        const description = tr.querySelector(".freight-cargo-desc")?.value.trim() || "";
        const dimensions = tr.querySelector(".freight-cargo-dimensions")?.value.trim() || "";
        const rowTotal = Math.max(0, quantity) * Math.max(0, unitValue);
        tr.querySelector(".freight-cargo-item-number").textContent = String(index + 1);
        tr.querySelector(".freight-cargo-item-total").textContent = rowTotal.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
        if (description || quantity > 0 || unitValue > 0 || dimensions) filled += 1;
        total += rowTotal;
      });
      const count = document.getElementById("freightCargoItemsCount");
      const totalLabel = document.getElementById("freightCargoItemsTotal");
      if (count) count.textContent = String(filled);
      if (totalLabel) totalLabel.textContent = total.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
    }

    function collectFreightCargoItems() {
      const rows = Array.from(document.querySelectorAll("#freightCargoItemsBody tr"));
      const items = [];
      const errors = [];
      rows.forEach((tr, index) => {
        const description = tr.querySelector(".freight-cargo-desc")?.value.trim() || "";
        const quantityRaw = tr.querySelector(".freight-cargo-qty")?.value.trim() || "";
        const unitRaw = tr.querySelector(".freight-cargo-unit")?.value.trim() || "";
        const dimensions = tr.querySelector(".freight-cargo-dimensions")?.value.trim() || "";
        const quantity = parseFreightDecimal(quantityRaw, 0);
        const unitValue = parseFreightDecimal(unitRaw, 0);
        const hasValue = Boolean(description || quantityRaw || unitRaw || dimensions);
        if (!hasValue) return;
        if (!description) errors.push(\`Item \${index + 1}: informe o nome / descrição.\`);
        if (!(quantity > 0)) errors.push(\`Item \${index + 1}: informe a quantidade maior que zero.\`);
        if (!(unitValue > 0)) errors.push(\`Item \${index + 1}: informe o valor unitário maior que zero.\`);
        items.push({
          id: tr.dataset.itemId || generateId(),
          description,
          quantity,
          unitValue,
          dimensions
    ...
    function freightCargoItemsTotal(items) {
      return normalizeFreightCargoItems(items).reduce((sum, item) => sum + (Number(item.quantity) || 0) * (Number(item.unitValue) || 0), 0);
    }

    function notaFiscalProductsFromFreight(freight) {
      return freightCargoItemsFromFreight(freight).map((item, index) => ({
        id: item.id || \`P-\${freight.id || Date.now()}-\${index + 1}\`,
        description: [item.description || "Carga vinculada a solicitacao de frete", formatFreightCargoItemDimensions(item)].filter(Boolean).join(" | "),
        quantity: Math.max(1, Number(item.quantity) || 1),
        unitValue: Math.max(0, Number(item.unitValue) || 0),
        dimensions: item.dimensions || legacyFreightDimensionsText(item),
        ncm: item.ncm || ""
      }));
    }

    function buildNotaFiscalRequestFromFreight(freight, existing, rows) {
      const now = new Date().toISOString();
      const id = linkedNotaFiscalIdForFreight(freight);
      const priority = getFreightUrgency(freight) === "URGENTE" ? "Urgente" : "Normal";
      const issuerDepartment = freight.numeroObra || freight.centroCusto || freight.obra || "";
      const issuerAddress = freight.enderecoColeta || freight.origem || freight.linkGoogleMapsColeta || "";
      const recipientAddress = freight.enderecoEntrega || freight.destino || freight.linkGoogleMapsEntrega || "";
      const carrierName = freight.transportadora || existing?.carrierName || "A definir pela cotacao do frete";
      const carrierCnpj = normalizeFreightCnpj(existing?.carrierCnpj || freight.fornecedorCnpj);
      const history = Array.isArray(existing?.history) ? [...existing.history] : [];
      history.push({
        at: now,
        text: existing
          ? \`Dados atualizados automaticamente pela solicitacao de frete \${freight.id}.\`
          : \`Solicitacao criada automaticamente pela solicitacao de frete \${freight.id}.\`
      });

      return {
        ...(existing || {}),
        id,
        code: existing?.code || freight.notaFiscalCodigo || nextNotaFiscalCode(rows),
        createdAt: existing?.createdAt || now,
        updatedAt: now,
        priority,
        deadline: existing?.deadline || String(freight.dataLimiteEntrega || "").slice(0, 10) || "",
        requesterName: freight.solicitante || existing?.requesterName || "",
        requesterEmail: freight.emailSolicitante || existing?.requesterEmail || "",
        requesterUserId: existing?.requesterUserId || "",
        issuerCnpj: existing?.issuerCnpj || "72.030.927/0005-09",
        issuerDepartment,
        issuerAddress,
        recipientCnpj: normalizeFreightCnpj(existing?.recipientCnpj || freight.fornecedorCnpj),
        recipientDepartment: freight.numeroObra || freight.obra || existing?.recipientDepartment || "",
        recipientAddress,
        freightResponsible: existing?.freightResponsible || "Transportadora",
        carrierName,
        carrierCnpj,
        products: notaFiscalProductsFromFreight(freight),
        status: existing?.status || NF_INITIAL_PHASE,
        phaseStartedAt: existing?.phaseStartedAt || now,
        phaseDurations: existing?.phaseDurations || { [NF_INITIAL_PHASE]: 0, [NF_PROGRESS_PHASE]: 0, [NF_DONE_PHASE]: 0 },
        completedAt: existing?.completedAt || null,
        internalNote: existing?.internalNote || "",
        linkedFreightId: freight.id,
        sourceModule: "fretes",
        freightIntegration: {
          sourceFreightId: freight.id,
          sourceFreightCode: freight.id,
          numeroRM: freight.numeroRM || "",
          numeroObra: freight.numeroObra || "",
          tipoMovimentacao: freight.tipoMovimentacao || freight.tipoFrete || "",
          coleta: issuerAddress,
          entrega: recipientAddress,
          nomeCarga: freight.nomeCarga || "",
          valorUnitarioCarga: freight.valorUnitarioCarga || "",
          itensCarga: normalizeFreightCargoItems(freight.itensCarga || freight.cargoItems || []),
          ...
          notaFiscalResposta: freight.notaFiscal || "",
          updatedAt: now
        },
        history
      };
    ...
    function upsertNotaFiscalFromFreight(freight) {
      if (!freightRequiresNotaFiscal(freight?.notaFiscal)) return null;
      const rows = loadNotaFiscalRequests();
      const linkedId = linkedNotaFiscalIdForFreight(freight);
      const existingIndex = rows.findIndex(row =>
        String(row?.id || "") === linkedId ||
        String(row?.linkedFreightId || "") === String(freight.id || "") ||
        String(row?.freightIntegration?.sourceFreightId || "") === String(freight.id || "")
      );
      const existing = existingIndex >= 0 ? rows[existingIndex] : null;
      const next = buildNotaFiscalRequestFromFreight(freight, existing, rows);
      if (existingIndex >= 0) rows[existingIndex] = next;
      else rows.unshift(next);
      saveNotaFiscalRequests(rows);
      return next;
    }
    ...
    function createFreight() {
      ...
      const movementValue = value("tipoMovimentacao");
      const documentationValue = value("notaFiscal");
      const isSupplierReturn = isSupplierReturnMovement(movementValue);
      const requiresSupplierFields = requiresSupplierFieldsForMovement(movementValue);
      const finalDocumentationValue = isSupplierReturn ? FREIGHT_RETURN_DOCUMENTATION_VALUE : documentationValue;
      if (!isSupplierReturn && isReturnDocumentationValue(documentationValue)) {
        alert(FREIGHT_DOCUMENTATION_DIVERGENCE_MESSAGE);
        document.getElementById("tipoMovimentacao")?.focus();
        updateSupplierReturnFormState(true);
        return;
      }
      if (requiresSupplierFields && (!value("fornecedorNome") || !value("fornecedorCnpj"))) {
        alert("Para devolução ou coleta no fornecedor, informe o nome do fornecedor e o CNPJ.");
        document.getElementById(!value("fornecedorNome") ? "fornecedorNome" : "fornecedorCnpj")?.focus();
        return;
      }

      const cargoItemsResult = collectFreightCargoItems();
      if (cargoItemsResult.errors.length) {
        alert("Revise os itens da carga:\\n\\n" + cargoItemsResult.errors.join("\\n"));
        document.getElementById("freightCargoItemsPanel")?.scrollIntoView({ behavior: "smooth", block: "center" });
        return;
      }
      const cargoItems = cargoItemsResult.items;
      const cargoItemsTotalValue = freightCargoItemsTotal(cargoItems);
      const firstCargoItem = cargoItems[0] || null;
      ...
      const formValues = {
        ...
        tipoMovimentacao: movementValue,
        ...
        notaFiscal: finalDocumentationValue,
        ...
        valorTotalCarga: value("valorTotalCarga") || (cargoItemsTotalValue > 0 ? String(cargoItemsTotalValue.toFixed(2)) : ""),
        nomeCarga: value("nomeCarga") || firstCargoItem?.description || "",
        valorUnitarioCarga: value("valorUnitarioCarga") || (firstCargoItem?.unitValue ? String(firstCargoItem.unitValue) : ""),
        itensCarga: cargoItems,
        cargoItems,
        totalPecas: value("totalPecas"),
        volumes: value("totalPecas"),
        fornecedorNome: requiresSupplierFields ? value("fornecedorNome") : "",
        fornecedorCnpj: requiresSupplierFields ? value("fornecedorCnpj") : "",
      ...
      if (isEditing) {
        ...
        const linkedNotaFiscal = upsertNotaFiscalFromFreight(existing);
        if (linkedNotaFiscal) {
          existing.notaFiscalSolicitacaoId = linkedNotaFiscal.id;
          existing.notaFiscalCodigo = linkedNotaFiscal.code;
          existing.notaFiscalIntegradaEm = linkedNotaFiscal.updatedAt;
        }
        saveFreights();
        const editedId = existing.id;
        document.getElementById("freightForm").reset();
        cancelFreightEdit(false);
        alert("Solicitacao atualizada com sucesso: " + editedId + (linkedNotaFiscal ? \`\\nNF de simples remessa atualizada: \${linkedNotaFiscal.code}\` : ""));
        renderAll();
        showTab("kanban");
        return;
      }
      ...
      const linkedNotaFiscal = upsertNotaFiscalFromFreight(freight);
      if (linkedNotaFiscal) {
        freight.notaFiscalSolicitacaoId = linkedNotaFiscal.id;
        freight.notaFiscalCodigo = linkedNotaFiscal.code;
        freight.notaFiscalIntegradaEm = linkedNotaFiscal.updatedAt;
      }
      ...
      document.getElementById("freightForm").reset();
      cancelFreightEdit(false);
      alert("Solicitacao criada com sucesso: " + freight.id + (linkedNotaFiscal ? \`\\nNF de simples remessa criada automaticamente: \${linkedNotaFiscal.code}\` : ""));
      renderAll();
    }
    ...
    function startFreightEdit(id) {
      ...
      renderFreightCargoItemRows(freightCargoItemsForDisplay(freight));
    ...
    function createFreightCard(freight) {
      ...
      card.innerHTML = \`
        ...
        <div class="freight-compact-actions">
          ...
          <button class="btn-secondary btn-small" type="button" data-action="delete" title="Excluir esta solicitação de frete">\${freightCardIcon("trash")}Excluir</button>
        </div>
      \`;
    ...
    function buildFreightQuoteEmailBody(freight) {
      ...
      const cargoItemLines = freightCargoItemsForDisplay(freight).map((item, index) => {
        const total = (Number(item.quantity) || 0) * (Number(item.unitValue) || 0);
        const dimensions = formatFreightCargoItemDimensions(item);
        return \`Item \${index + 1}: \${quoteEmailValue(item.description)} | Qtd: \${item.quantity || 0} | Unit.: \${formatCurrency(item.unitValue)} | Total: \${formatCurrency(total)}\${dimensions ? \` | \${dimensions}\` : ""}\`;
      });
      ...
      const lines = [
        ...
        "📦 DETALHES DA CARGA",
        "",
        ...(cargoItemLines.length ? [...cargoItemLines, ""] : []),
        \`Nome / descrição da carga: \${quoteEmailValue(freight.nomeCarga || freight.materiais || freight.tipoMaterial)}\`,
        ...
        \`Valor unitário da carga: \${quoteEmailCurrency(freight.valorUnitarioCarga)}\`,
      ...
      document.getElementById("modalContent").innerHTML = \`
        ...
        <div class="panel" style="box-shadow:none; border:1px solid #e5e7eb;">
          ...
          <div class="form-grid">
            ...
            \${detail("Solicitacao NF integrada", freight.notaFiscalCodigo || freight.notaFiscalSolicitacaoId || "-")}
            \${detail("Peso total kg", freight.pesoTotalCarga || freight.peso)}
            \${detail("Valor total carga", formatCurrency(freight.valorTotalCarga))}
            \${detail("Nome / descriÃ§Ã£o da carga", freight.nomeCarga || "-", "full")}
            \${detail("Valor unitÃ¡rio carga", freight.valorUnitarioCarga ? formatCurrency(freight.valorUnitarioCarga) : "-")}
            \${detail("Itens da carga para NF", formatFreightCargoItemsText(freightCargoItemsForDisplay(freight)), "full")}
