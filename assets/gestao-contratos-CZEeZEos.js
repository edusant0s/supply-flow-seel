...
.kpi-card{background:var(--card);border:1px solid var(--border);border-top:5px solid var(--yellow);border-radius:24px;padding:18px 18px 16px;box-shadow:0 10px 26px rgba(0,83,131,.08)}
...
.analytics-card,.toolbar-panel{background:var(--card);border:1px solid var(--border);border-top:5px solid var(--yellow);border-radius:24px;padding:18px 20px;box-shadow:0 12px 28px rgba(0,83,131,.08)}
...
.timer{background:var(--card);border:1px solid #cfdde7;border-radius:16px;padding:10px 12px;display:grid;gap:5px;margin-top:10px}.stage-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.stage-actions button{min-height:34px;border-radius:14px;font-size:14px}.stage-actions .secondary{background:var(--card)}
...
.detail-summary-tile--red strong{color:#e7000b}
.detail-summary-tile--yellow strong{color:#d97706}
.detail-summary-tile--green strong{color:#059669}
.detail-summary-tile--gray strong{color:#5c7888}
...
.contract-icon{vertical-align:-2px}
.detail-edit-actions button,.detail-comment-form button{display:inline-flex;align-items:center;gap:6px}
.detail-modal .contract-card-observation{
  display:grid;
  gap:8px;
  margin:0 0 16px;
  padding:14px;
...
.detail-modal .contract-card-observation label{
  margin:0;
  color:#6c5600;
  font-size:12px;
  font-weight:900;
...
.detail-modal .contract-card-observation textarea{
  width:100%;
  min-height:90px;
  max-height:220px;
  margin:0;
...
.detail-modal .contract-card-observation textarea:focus{
  border-color:#075985;
  box-shadow:0 0 0 3px rgba(7,89,133,.14);
  outline:none;
}
.detail-modal .contract-observation-footer{
  display:flex;
  justify-content:space-between;
  align-items:center;
  gap:10px;
}
.detail-modal .contract-observation-save-status{
  color:#816b10;
  font-size:12px;
  font-weight:700;
}
.detail-modal .contract-observation-save-status.saved{color:#15803d}
.detail-modal .contract-observation-counter{
  color:#6c5600;
  font-size:12px;
  font-weight:900;
  font-variant-numeric:tabular-nums;
...
html[data-theme="dark"]{
  ...
  --sf-text:#e4edf0;
  --sf-muted:#84a4b1;
  --sf-shadow:0 12px 32px rgba(0,0,0,.36);
  --card:#000e1d;
}
...
.sf-topbar{height:68px;background:var(--card);border-bottom:1px solid var(--sf-line);display:flex;align-items:center;justify-content:space-between;padding:0 30px;position:sticky;top:0;z-index:45;box-shadow:0 3px 16px rgba(15,53,80,.04)}
...
.sf-icon-btn{width:40px;height:40px;min-height:40px;padding:0;border-radius:12px;background:var(--card);color:var(--sf-navy);border:1px solid var(--sf-line);display:grid;place-items:center}.sf-icon-btn svg,.sf-primary-action svg{width:19px;height:19px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.sf-menu-btn{display:none}.sf-notification{position:relative}.sf-notification span{position:absolute;right:8px;top:7px;width:7px;height:7px;background:#ef4444;border:2px solid #fff;border-radius:50%}
.sf-global-search{height:40px;min-width:270px;border:1px solid var(--sf-line);border-radius:12px;background:var(--card);display:flex;align-items:center;gap:9px;padding:0 12px}.sf-global-search svg{width:18px;height:18px;fill:none;stroke:#7b909e;stroke-width:2}.sf-global-search input{border:0;background:transparent;padding:0;margin:0;box-shadow:none;font-size:13px}.sf-global-search input:focus{box-shadow:none}
...
.sf-module-tabs{margin:0 34px;background:var(--card);border:1px solid var(--sf-line);border-radius:14px;padding:5px;display:flex;gap:4px;box-shadow:0 6px 20px rgba(15,53,80,.05)}
.sf-module-tabs .tab{min-height:40px;border-radius:10px;color:#617989;padding:0 16px;background:transparent;display:flex;align-items:center;gap:8px;font-size:13px}.sf-module-tabs .tab:hover{background:var(--card);color:var(--sf-navy)}.sf-module-tabs .tab.active{background:var(--card);color:var(--sf-blue);box-shadow:inset 0 -2px 0 var(--sf-blue)}.sf-tab-dot{width:7px;height:7px;border-radius:50%;background:#b7c6cf}.sf-module-tabs .tab.active .sf-tab-dot{background:var(--sf-yellow);box-shadow:0 0 0 3px rgba(245,196,0,.2)}
...
.panel{padding:24px;border-radius:18px}.head{padding-bottom:18px;margin-bottom:20px;border-bottom:1px solid var(--sf-line);align-items:center}.head h2{font-size:22px}.head .hint{font-size:13px}.pill{background:var(--card);color:var(--sf-blue);border:1px solid #d8e8f0;border-bottom:0;border-radius:999px;padding:8px 12px;font-size:11px;font-weight:800}
.stats{gap:14px}.stat{border-top:0;border-left:4px solid var(--sf-yellow);border-radius:14px;padding:14px 15px;box-shadow:none;background:var(--card)}.stat strong{font-size:20px}.section{border-radius:15px;border:1px solid var(--sf-line);box-shadow:none;margin-bottom:16px}.section-title{background:var(--card);color:var(--sf-navy);border-bottom:1px solid var(--sf-line);padding:13px 16px;font-size:14px;letter-spacing:.01em}.section-title:before{content:"";display:inline-block;width:4px;height:16px;background:var(--sf-yellow);border-radius:3px;vertical-align:-3px;margin-right:9px}.section-body{padding:18px 18px 20px}
label{font-size:12px;color:#365166}.grid{gap:15px 16px}input,select,textarea{border-radius:10px;border-color:#cedbe3;background:var(--card);padding:10px 11px;font-size:13px}input:hover,select:hover,textarea:hover{border-color:#aebfca}input:focus,select:focus,textarea:focus{border-color:var(--sf-blue);box-shadow:0 0 0 3px rgba(7,89,133,.1)}.note{background:var(--card);border-left-color:var(--sf-yellow)}
.actions{padding-top:16px;border-top:1px solid var(--sf-line)}button{border-radius:10px;font-size:13px}.primary{background:var(--sf-navy);border-bottom:3px solid var(--sf-yellow)}.secondary{color:var(--sf-navy);background:var(--card)}
...
.editor-side,.editor-main{border-radius:15px;box-shadow:none}.editor-section-card,.editor-question-card{border-radius:11px}.editor-section-card.active{background:var(--card)}
...
.import-card{background:var(--card);border:1px solid var(--sf-line);border-radius:16px;padding:18px;box-shadow:0 8px 24px rgba(15,53,80,.05)}
...
.mapping-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.mapping-field{background:var(--card);border:1px solid var(--sf-line);border-radius:12px;padding:10px}.mapping-field small{display:block;color:#8296a3;font-size:9px;margin-top:4px;line-height:1.35}.mapping-required:after{content:' *';color:#dc2626}
.preview-wrap{margin-top:18px;border:1px solid var(--sf-line);border-radius:15px;overflow:hidden;background:var(--card)}.preview-head{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px 14px;background:var(--card);border-bottom:1px solid var(--sf-line)}.preview-head h3{font-size:14px;color:var(--sf-navy);margin:0}.preview-head span{font-size:10px;color:var(--sf-muted)}.preview-scroll{overflow:auto;max-height:360px}.import-preview{width:100%;border-collapse:collapse;min-width:760px;font-size:11px}.import-preview th{position:sticky;top:0;background:var(--card);color:#526c7c;text-align:left;padding:9px 10px;border-bottom:1px solid var(--sf-line);white-space:nowrap;z-index:1}.import-preview td{padding:9px 10px;border-bottom:1px solid #edf2f5;color:#29465a;max-width:230px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.import-preview tr:hover td{background:var(--card)}.import-empty{padding:42px 18px;text-align:center;color:var(--sf-muted);font-size:12px}
...
.dashboard-filter-panel{background:var(--card);border:1px solid var(--sf-line);border-radius:16px;padding:18px;box-shadow:var(--sf-shadow)}
...
.dashboard-filter-summary{background:var(--card);color:var(--sf-blue);border:1px solid #d8e8f0;border-radius:999px;padding:7px 11px;font-size:11px;font-weight:900;white-space:nowrap}
...
#kanbanView .contract-icon{display:block !important;flex-shrink:0 !important}
...
#kanbanView .contract-mini-badge.traffic-gray{background:var(--card) !important;color:#475569 !important}
...
#kanbanView .contract-deadline-detail.gray{background:var(--card) !important;color:#475569 !important;border:1px solid #cbd5e1 !important}
...
#kanbanView .col-icon{
  ...
  box-shadow:0 5px 12px rgba(10,46,61,.15) !important;
}
#kanbanView .unified-col.process-two .col-icon{
...
html[data-theme="dark"] #kanbanView .sla-row strong{
...
#kanbanView .contract-freight-card{
  ...
  border-left:5px solid #1b6d8e !important;
  border-radius:14px !important;
  background:var(--card) !important;
  box-shadow:0 8px 18px rgba(10,46,61,.1) !important;
  cursor:pointer !important;
}
#kanbanView .contract-freight-card.red{border-left-color:#e7000b !important}
#kanbanView .contract-freight-card.yellow{border-left-color:#d97706 !important}
#kanbanView .contract-freight-card.green{border-left-color:#059669 !important}
#kanbanView .contract-freight-card.gray{border-left-color:#94a3b8 !important}
#kanbanView .contract-freight-card:hover{transform:none !important;box-shadow:0 10px 22px rgba(10,46,61,.16) !important}
#kanbanView .contract-compact-top{
  display:grid !important;
  grid-template-columns:minmax(0,1fr) auto !important;
  gap:8px !important;
...
#kanbanView .contract-priority-bubble.urgent{background:rgba(217,119,6,.14) !important;color:#d97706 !important}
#kanbanView .contract-priority-bubble.normal{background:rgba(27,109,142,.12) !important;color:#1b6d8e !important}
#kanbanView .contract-status-chip{
...
#kanbanView .contract-status-chip--nao-iniciado{background:rgba(245,158,11,.12) !important;color:#92400e !important}
#kanbanView .contract-status-chip--em-andamento{background:rgba(14,165,233,.12) !important;color:#075985 !important}
#kanbanView .contract-status-chip--finalizado{background:rgba(16,185,129,.12) !important;color:#047857 !important}
#kanbanView .contract-value-pill{
  min-height:22px !important;
  display:inline-flex !important;
  align-items:center !important;
...
#kanbanView .contract-attachment-count{
  min-height:22px !important;
  display:inline-flex !important;
  align-items:center !important;
  gap:4px !important;
...
#kanbanView .contract-comment-badge{
  min-height:22px !important;
  display:inline-flex !important;
  align-items:center !important;
  gap:4px !important;
  ...
  background:rgba(27,109,142,.12) !important;
  font-size:8.5px !important;
  font-weight:850 !important;
  white-space:nowrap !important;
}
#kanbanView .contract-route-pill{
  display:flex !important;
  flex-wrap:wrap !important;
  gap:5px !important;
  border:1px solid var(--border) !important;
...
#kanbanView .contract-route-pill span:not(:last-child)::after{content:"|" !important;margin-left:5px !important;color:var(--border) !important}
#kanbanView .contract-deadline-strip{
  min-height:28px !important;
  display:flex !important;
  align-items:center !important;
  justify-content:space-between !important;
...
#kanbanView .contract-deadline-strip strong{font-size:9.5px !important;white-space:nowrap !important}
#kanbanView .contract-deadline-strip--red{border-color:rgba(231,0,11,.42) !important;background:rgba(231,0,11,.09) !important}
#kanbanView .contract-deadline-strip--yellow{border-color:rgba(217,119,6,.58) !important;background:rgba(217,119,6,.12) !important}
#kanbanView .contract-deadline-strip--green{border-color:rgba(5,150,105,.34) !important;background:rgba(5,150,105,.08) !important}
#kanbanView .contract-deadline-strip--gray{border-color:var(--border) !important;background:var(--bg) !important}
#kanbanView .contract-compact-lines b{color:var(--text) !important;font-weight:950 !important}
#kanbanView .card.contract-freight-card{cursor:grab !important}
#kanbanView .contract-compact-lines{
  display:grid !important;
  gap:4px !important;
  margin-top:6px !important;
...
#kanbanView .contract-compact-lines > div > span:last-child{
  overflow:hidden !important;
  text-overflow:ellipsis !important;
  white-space:nowrap !important;
}
#kanbanView .contract-line-icon{color:#1b6d8e !important;font-size:8px !important;text-align:center !important;display:inline-flex !important;align-items:center !important;justify-content:center !important}
#kanbanView .contract-compact-description{
  min-height:0 !important;
...
#kanbanView .contract-responsible-note b{font-size:8px !important;line-height:1.1 !important}
#kanbanView .contract-responsible-note span{
  font-size:9px !important;
  line-height:1.25 !important;
  display:-webkit-box !important;
  -webkit-line-clamp:2 !important;
...
#kanbanView .contract-mini-badge.urgent{background:rgba(217,119,6,.12) !important;color:#d97706 !important}
#kanbanView .contract-mini-badge.normal{background:rgba(27,109,142,.1) !important;color:#1b6d8e !important}
#kanbanView .contract-mini-badge.traffic-red{background:rgba(231,0,11,.1) !important;color:#e7000b !important}
#kanbanView .contract-mini-badge.traffic-yellow{background:rgba(217,119,6,.12) !important;color:#d97706 !important}
#kanbanView .contract-mini-badge.traffic-green{background:rgba(5,150,105,.1) !important;color:#059669 !important}
#kanbanView .contract-mini-badge.traffic-gray{background:var(--card) !important;color:#475569 !important}
#kanbanView .contract-mini-badge.process-one{background:rgba(27,109,142,.1) !important;color:#1b6d8e !important}
#kanbanView .contract-mini-badge.process-two{background:rgba(10,46,61,.08) !important;color:#0a2e3d !important}
#kanbanView .contract-mini-badge.kind{background:rgba(217,119,6,.12) !important;color:#d97706 !important}
#kanbanView .contract-compact-meta{
  display:grid !important;
  grid-template-columns:minmax(0,1fr) auto !important;
  gap:6px !important;
...
#kanbanView .contract-compact-meta > span:first-child{
  overflow:hidden !important;
  text-overflow:ellipsis !important;
  white-space:nowrap !important;
}
#kanbanView .contract-compact-meta strong{color:var(--text) !important}
#kanbanView .contract-meta-status{font-weight:900 !important;white-space:nowrap !important}
#kanbanView .contract-meta-status.red{color:#e7000b !important}
#kanbanView .contract-meta-status.yellow{color:#d97706 !important}
#kanbanView .contract-meta-status.green{color:#059669 !important}
#kanbanView .contract-meta-status.gray{color:#64748b !important}
#kanbanView .contract-compact-timer{
  display:grid !important;
  gap:4px !important;
...
#kanbanView .contract-compact-timer > div > span:first-child{
  overflow:hidden !important;
  text-overflow:ellipsis !important;
  white-space:nowrap !important;
}
...
#kanbanView .contract-phase-selector-wrap{display:grid !important;gap:3px !important;margin-top:6px !important}
#kanbanView .contract-phase-selector-wrap label{
  color:var(--muted) !important;
  font-size:8px !important;
  line-height:1 !important;
...
#kanbanView .contract-phase-selector-wrap .contract-error-select{
  width:100% !important;
  height:29px !important;
  min-height:29px !important;
  padding:0 8px !important;
  border:1px solid rgba(231,0,11,.34) !important;
  border-radius:8px !important;
  background:var(--card) !important;
  color:var(--text) !important;
  font-size:9.5px !important;
...
#kanbanView .contract-compact-actions button:hover{
  background:var(--bg) !important;
}
</style>
...
.contract-report-kpi{padding:13px 14px;border:1px solid #dce7ed;border-left:4px solid #ffdd00;border-radius:13px;background:var(--card)}
...
.contract-report-filters{display:grid;grid-template-columns:minmax(250px,1.5fr) repeat(3,minmax(150px,.75fr)) auto;gap:10px;align-items:end;padding:15px 16px;border:1px solid #dfe7ed;border-radius:15px;background:var(--card);box-shadow:0 8px 22px rgba(15,53,80,.05)}
...
.contract-report-table-card{overflow:hidden;border:1px solid #dfe7ed;border-radius:16px;background:var(--card);box-shadow:0 10px 26px rgba(15,53,80,.06)}
.contract-report-table-head{display:flex;justify-content:space-between;align-items:center;gap:14px;padding:15px 17px;border-bottom:1px solid #dfe7ed;background:var(--card)}
...
.contract-report-table thead th{position:sticky;top:0;z-index:2;padding:11px 10px;background:var(--card);color:#456273;text-align:left;font-size:9px;font-weight:950;letter-spacing:.035em;text-transform:uppercase;white-space:nowrap;border-bottom:1px solid #cfdee6}
...
.contract-report-table tbody tr:hover td{background:var(--card)}
...
#kanbanView .contract-compact-actions .contract-edit-btn{
  background:var(--card) !important;
  color:#1b6d8e !important;
  border:1px solid rgba(27,109,142,.4) !important;
}
#kanbanView .contract-compact-actions .contract-edit-btn:hover{
  background:var(--card) !important;
}
#kanbanView .contract-compact-actions .contract-delete-btn{
  background:var(--card) !important;
  color:#b42318 !important;
  border:1px solid #f1b4ae !important;
}
#kanbanView .contract-compact-actions .contract-delete-btn:hover{
...
<style id="contract-insights-styles">
.contract-insight-section{
  display:grid;
  gap:14px;
  border:1px solid var(--border);
  border-radius:18px;
  background:var(--card);
  box-shadow:0 9px 22px rgba(0,62,95,.08);
  padding:16px;
  margin-bottom:16px;
}
.contract-insight-section:empty{display:none}
.contract-insight-title{display:flex;align-items:center;gap:10px}
.contract-insight-title>span{
  width:34px;height:34px;flex:0 0 auto;
  display:inline-flex;align-items:center;justify-content:center;
  border-radius:10px;background:#1b6d8e;color:#fcc800;font-size:16px;
  box-shadow:0 10px 24px rgba(10,46,61,.13);
}
.contract-insight-title h3{margin:0;color:var(--text);font-size:1.05rem}
.contract-insight-title p{margin:3px 0 0;color:var(--muted);font-size:.78rem;font-weight:800}

.contract-insight-summary{display:grid;grid-template-columns:minmax(260px,1.4fr) repeat(3,minmax(150px,.6fr));gap:12px}
.contract-insight-summary-card{
  min-width:0;min-height:100px;display:grid;align-content:center;gap:5px;
  border:1px solid var(--border);border-top:4px solid var(--insight-accent,#fcc800);
  border-radius:13px;background:var(--card);box-shadow:0 8px 18px rgba(10,46,61,.08);padding:12px;
}
.contract-insight-summary-card--wide{grid-template-columns:38px minmax(0,1fr);align-items:center;gap:11px}
.contract-insight-summary-card--danger{--insight-accent:#e7000b}
.contract-insight-summary-card--warning{--insight-accent:#d97706}
.contract-insight-summary-card--success{--insight-accent:#059669}
.contract-insight-summary-icon{
  width:38px;height:38px;border-radius:12px;font-size:18px;
  display:inline-flex;align-items:center;justify-content:center;
  background:color-mix(in srgb,var(--insight-accent,#fcc800) 14%,#fff);
  color:var(--insight-accent,#8a6d00);
}
.contract-insight-summary-card small{color:var(--muted);font-size:.65rem;font-weight:950;text-transform:uppercase}
.contract-insight-summary-card h4{margin:0;color:var(--text);font-size:.94rem;line-height:1.2}
.contract-insight-summary-card strong{color:var(--text);font-size:1.4rem;line-height:1}
.contract-insight-summary-card p,.contract-insight-summary-card time{margin:0;color:var(--muted);font-size:.72rem;font-weight:800;line-height:1.35}
.contract-insight-summary-card time{display:block;font-size:.62rem;margin-top:2px}

.contract-insight-grid-head{display:flex;align-items:flex-end;justify-content:space-between;gap:10px}
.contract-insight-grid-head h4{margin:0;color:var(--text);font-size:.92rem}
.contract-insight-grid-head span{color:var(--muted);font-size:.7rem;font-weight:850;text-align:right}

.contract-insight-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}
.contract-insight-card{
  min-width:0;display:grid;align-content:start;gap:7px;
  border:1px solid var(--border);border-top:4px solid var(--insight-accent,#fcc800);
  border-radius:13px;background:var(--card);box-shadow:0 8px 18px rgba(10,46,61,.08);padding:12px;
}
.contract-insight-card--danger{--insight-accent:#e7000b}
.contract-insight-card--warning{--insight-accent:#d97706}
.contract-insight-card--monitor{--insight-accent:#94a3b8}
.contract-insight-card--success{--insight-accent:#059669}
.contract-insight-card-head{display:grid;grid-template-columns:30px minmax(0,1fr) auto;gap:8px;align-items:center}
.contract-insight-card-icon{
  width:30px;height:30px;border-radius:10px;font-size:15px;
  display:inline-flex;align-items:center;justify-content:center;
  background:color-mix(in srgb,var(--insight-accent,#fcc800) 14%,#fff);
}
.contract-insight-card-head small{display:block;color:var(--muted);font-size:.62rem;font-weight:950;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.contract-insight-card-head strong{display:block;color:var(--text);font-size:.85rem;line-height:1.2}
.contract-insight-priority{
  border-radius:999px;padding:3px 7px;font-size:.6rem;font-style:normal;font-weight:950;white-space:nowrap;
  background:color-mix(in srgb,var(--insight-accent,#fcc800) 14%,#fff);color:var(--insight-accent,#8a6d00);
}
.contract-insight-card-value{display:flex;align-items:baseline;gap:6px;flex-wrap:wrap}
.contract-insight-card-value strong{color:var(--text);font-size:1.3rem;font-weight:950;line-height:1}
.contract-insight-card-value span{color:var(--muted);font-size:.7rem;font-weight:800}
.contract-insight-card p{margin:0;color:var(--muted);font-size:.72rem;font-weight:800;line-height:1.4}
.contract-insight-card footer{margin-top:2px;padding-top:7px;border-top:1px solid var(--border);color:var(--text);font-size:.72rem;font-weight:800;line-height:1.4}
.contract-insight-card footer b{font-weight:950}

@media(max-width:1300px){.contract-insight-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.contract-insight-summary{grid-template-columns:1fr 1fr}}
@media(max-width:700px){.contract-insight-grid{grid-template-columns:1fr}.contract-insight-summary{grid-template-columns:1fr}}
</style>
...
<section class="hidden" id="dashboardView">
  <div class="kanban-shell">
    ...
    <section class="contract-insight-section" id="contractInsightsSection"></section>
...
const CONTRACT_ICON_PATHS={
  ...
  calendar:'<rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/><path d="M8 14h.01"/><path d="M12 14h.01"/><path d="M16 14h.01"/>',
...
function contractIcon(name,size){
  const path=CONTRACT_ICON_PATHS[name];
  if(!path)return '';
  const px=size||14;
  return \`<svg class="contract-icon" width="\${px}" height="\${px}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">\${path}</svg>\`;
}
...
function renderKanbanDashboard(list){
  ...
  renderContractInsights(list);
}
function pctShare(part,total){if(!total)return 0;return Math.round((part/total)*100)}
function toneRank(tone){return {danger:0,warning:1,monitor:2,success:3}[tone]}
function toneLabel(tone){return {danger:'Crítico',warning:'Atenção',monitor:'Monitorar',success:'Controlado'}[tone]}
function parseContractValueNumber(r){
  const raw=contractValue(r);
  if(!raw)return 0;
  const n=Number(String(raw).replace(/[^\\d,.-]/g,'').replace(/\\.(?=\\d{3}(\\D|$))/g,'').replace(',','.'));
  return Number.isFinite(n)?n:0;
}
function topShareBy(items,keyFn){
  const map=new Map();
  items.forEach(item=>{
  ...
  return {name:top[0],share:pctShare(top[1],items.length),count:top[1]};
}
function buildContractInsights(list){
  const total=list.length;
  if(!total){
    return {
      total:0,healthScore:100,headlineTone:'success',headlineIcon:'✅',
      headline:'Sem dados no filtro atual',
      headlineText:'Ajuste os filtros para visualizar os alertas da carteira de contratos.',
      criticalCount:0,warningCount:0,exposure:0,cards:[]
    };
  }

  const open=list.filter(r=>r.status!=='Aprovado no Compor');
  const finished=list.filter(r=>r.status==='Aprovado no Compor');
  const late=open.filter(r=>deadlineInfo(r).status==='Em atraso');
  const alertItems=open.filter(r=>deadlineInfo(r).status==='Em alerta');
  const noDeadline=open.filter(r=>deadlineInfo(r).status==='Sem prazo');
  const withError=list.filter(contractRequestHasError);
  const urgent=list.filter(r=>String(r.urgencia||'').toUpperCase().includes('URGENTE'));
  const noValue=list.filter(r=>!contractValue(r));

  const exposure=[...late,...alertItems].reduce((sum,r)=>sum+parseContractValueNumber(r),0);

  const finishRate=pctShare(finished.length,total);
  const openShare=pctShare(open.length,total);
  const errorShare=pctShare(withError.length,total);
  const noDeadlineShare=pctShare(noDeadline.length,open.length||1);
  const urgentShare=pctShare(urgent.length,total);

  const centerShare=topShareBy(open,r=>String(r.centro||r.data?.centro_obra||r.data?.centro_departamento||'').trim());
  const typeShare=topShareBy(list,r=>String(r.tipo||'Não informado'));

  const stageAverages=averageTimeByStage(list).filter(s=>s.count>0);
  const bottleneck=stageAverages.slice().sort((a,b)=>b.value-a.value)[0]||null;

  const oldestOpen=open.slice().sort((a,b)=>totalSlaMs(b)-totalSlaMs(a))[0]||null;
  const oldestMs=oldestOpen?totalSlaMs(oldestOpen):0;
  const dayMs=24*60*60*1000;

  let healthScore=100;
  healthScore-=Math.min(30,pctShare(late.length,total)*1.3);
  healthScore-=Math.min(14,pctShare(alertItems.length,total)*0.6);
  healthScore-=Math.min(12,noDeadlineShare*0.5);
  healthScore-=Math.min(12,errorShare*0.6);
  healthScore-=Math.min(14,Math.max(0,60-finishRate)*0.3);
  if(centerShare.share>=70)healthScore-=6;else if(centerShare.share>=50)healthScore-=3;
  healthScore=Math.max(0,Math.min(100,Math.round(healthScore)));

  let headlineTone='success',headlineIcon='✅',
    headline='Fluxo de contratos sob controle',
    headlineText='Não há atrasos críticos ou pendências relevantes no filtro atual.';

  if(late.length){
    headlineTone='danger';headlineIcon='⛔';
    headline='Atraso de prazo exige atuação imediata';
    headlineText=\`\${late.length} solicitação(ões) em aberto \${late.length===1?'está':'estão'} com o prazo de atendimento vencido, com exposição estimada de \${currencyFmt(exposure)}.\`;
  }else if(alertItems.length||noDeadline.length){
    headlineTone='warning';headlineIcon='⏳';
    headline='Prazos exigem atenção nos próximos dias';
    headlineText=\`\${alertItems.length} solicitação(ões) vencem em breve e \${noDeadline.length} estão sem prazo definido.\`;
  }else if(errorShare>=15){
    headlineTone='warning';headlineIcon='⚑';
    headline='Qualidade das solicitações pede revisão';
    headlineText=\`\${withError.length} solicitação(ões) estão marcadas com erro pelo responsável (\${errorShare}% da carteira).\`;
  }else if(openShare>=65){
    headlineTone='warning';headlineIcon='📥';
    headline='Carteira em aberto acima do esperado';
    headlineText=\`\${openShare}% das solicitações filtradas ainda estão em aberto.\`;
  }

  const cards=[
    {key:'late',tone:late.length?'danger':'success',icon:'⛔',eyebrow:'Risco de prazo',title:'Solicitações com prazo vencido',
      value:String(late.length),meta:\`\${pctShare(late.length,total)}% da carteira - \${currencyFmt(exposure)} em exposição\`,
      description:late.length?'Há solicitações em aberto com o prazo de atendimento já vencido.':'Nenhuma solicitação em aberto está com prazo vencido.',
      action:late.length?'Cobrar responsáveis e atualizar a fase no Kanban.':'Manter rotina diária de conferência dos prazos.'},
    {key:'alert',tone:alertItems.length?'warning':'success',icon:'⏳',eyebrow:'Próximos dias',title:'Solicitações vencendo em breve',
      value:String(alertItems.length),meta:\`\${pctShare(alertItems.length,total)}% da carteira filtrada\`,
      description:alertItems.length?'Janela curta para concluir a etapa atual antes do vencimento do prazo.':'Nenhuma solicitação com vencimento iminente.',
      action:alertItems.length?'Priorizar a validação/assinatura dessas solicitações.':'Manter acompanhamento semanal do funil.'},
    {key:'no-deadline',tone:noDeadline.length?'warning':'success',icon:'📥',eyebrow:'Qualidade de dados',title:'Solicitações sem prazo definido',
      value:String(noDeadline.length),meta:\`\${noDeadlineShare}% do que está em aberto\`,
      description:noDeadline.length?'A ausência do prazo de atendimento impede alertas confiáveis de SLA.':'Todas as solicitações em aberto possuem prazo de atendimento informado.',
      action:noDeadline.length?'Completar a data limite de atendimento de cada solicitação aberta.':'Manter o preenchimento obrigatório do prazo na abertura.'},
    {key:'error',tone:withError.length?(errorShare>=15?'danger':'warning'):'success',icon:'⚑',eyebrow:'Qualidade das solicitações',title:'Solicitações marcadas com erro',
      value:String(withError.length),meta:\`\${errorShare}% da carteira filtrada\`,
      description:withError.length?'Solicitações foram marcadas pelo responsável como contendo erro no formulário.':'Nenhuma solicitação foi marcada com erro no filtro atual.',
      action:withError.length?'Corrigir ou orientar novo envio das solicitações marcadas.':'Manter a checagem do formulário na triagem inicial.'},
    {key:'finish-rate',tone:finishRate<35?'danger':finishRate<60?'warning':'success',icon:'✅',eyebrow:'Eficiência de fluxo',title:'Taxa de finalização da carteira',
      value:\`\${finishRate}%\`,meta:\`\${finished.length} de \${total} solicitação(ões)\`,
      description:finishRate<60?'Parcela relevante das solicitações ainda não chegou ao fim do fluxo.':'A maior parte da carteira já foi finalizada no Compor.',
      action:finishRate<60?'Revisar gargalos das etapas e priorizar as solicitações mais antigas.':'Manter a cadência atual de finalização.'},
    {key:'center-concentration',tone:centerShare.share>=70?'danger':centerShare.share>=50?'warning':'success',icon:'🏗️',eyebrow:'Concentração operacional',title:'Dependência de uma obra/centro de custo',
      value:centerShare.name?\`\${centerShare.share}%\`:'-',meta:centerShare.name?\`\${esc(centerShare.name)} - \${centerShare.count} solicitação(ões) em aberto\`:'Sem centro de custo informado no filtro',
      description:centerShare.share>=50?'A carteira em aberto está concentrada em poucas obras/centros de custo.':'A carteira em aberto está distribuída entre obras e centros de custo.',
      action:centerShare.share>=50?'Avaliar se a obra concentradora precisa de apoio dedicado.':'Manter o acompanhamento por obra/centro de custo.'},
    {key:'type-mix',tone:typeShare.share>=70?'monitor':'success',icon:'📄',eyebrow:'Mix de demanda',title:'Concentração por tipo de documento',
      value:typeShare.name?\`\${typeShare.share}%\`:'-',meta:typeShare.name?esc(typeShare.name):'Sem tipo informado',
      description:typeShare.share>=70?'A demanda filtrada está concentrada em um único tipo de documento.':'A demanda filtrada está distribuída entre os tipos de documento.',
      action:typeShare.share>=70?'Avaliar se a concentração de tipo exige capacidade dedicada.':'Manter o acompanhamento periódico do mix de demanda.'},
    {key:'urgency',tone:urgentShare>=40?'warning':'success',icon:'🚨',eyebrow:'Prioridade',title:'Solicitações urgentes',
      value:String(urgent.length),meta:\`\${urgentShare}% da carteira filtrada\`,
      description:urgentShare>=40?'Parcela elevada da carteira está marcada como urgente, pressionando o SLA de 3 dias úteis.':'O volume de solicitações urgentes está em nível administrável.',
      action:urgentShare>=40?'Confirmar critério de urgência e redistribuir capacidade.':'Manter o SLA diferenciado para urgências.'},
    {key:'bottleneck',tone:bottleneck?'monitor':'success',icon:'🧭',eyebrow:'Gargalo de etapa',title:'Etapa com maior tempo médio',
      value:bottleneck?fmtDuration(bottleneck.value):'-',meta:bottleneck?\`\${esc(bottleneck.stage)} - \${bottleneck.count} solicitação(ões)\`:'Sem tempo suficiente registrado',
      description:bottleneck?\`A etapa "\${bottleneck.stage}" concentra o maior tempo médio de permanência no filtro atual.\`:'Ainda não há tempo suficiente registrado por etapa.',
      action:bottleneck?'Investigar a causa da demora nesta etapa e redistribuir responsáveis se necessário.':'Manter o acompanhamento do tempo por etapa.'},
    {key:'oldest-open',tone:oldestMs>10*dayMs?'danger':oldestMs>5*dayMs?'warning':'success',icon:'⏱️',eyebrow:'Prioridade de fila',title:'Maior tempo em aberto na fila',
      value:oldestOpen?fmtDuration(oldestMs):'-',meta:oldestOpen?esc(requestTitle(oldestOpen)||oldestOpen.id):'Sem solicitações em aberto',
      description:oldestOpen?'Esta é a solicitação em aberto mais antiga do filtro atual.':'Não há solicitações em aberto no filtro atual.',
      action:oldestOpen?'Priorizar esta solicitação antes de iniciar novas tratativas.':'Manter o controle de entrada da fila.'},
    {key:'no-value',tone:noValue.length?'monitor':'success',icon:'💰',eyebrow:'Visibilidade financeira',title:'Solicitações sem valor informado',
      value:String(noValue.length),meta:\`\${pctShare(noValue.length,total)}% da carteira filtrada\`,
      description:noValue.length?'Solicitações sem valor contratual informado reduzem a visibilidade financeira da carteira.':'Todas as solicitações do filtro possuem valor contratual informado.',
      action:noValue.length?'Completar o valor do contrato/aditivo nos formulários pendentes.':'Manter o preenchimento do valor na abertura.'},
    {key:'exposure',tone:exposure>0?'warning':'success',icon:'💸',eyebrow:'Exposição financeira',title:'Valor em risco (atrasados + em alerta)',
      value:currencyFmt(exposure),meta:\`\${late.length+alertItems.length} solicitação(ões) nesta condição\`,
      description:exposure>0?'Soma do valor contratual das solicitações atrasadas ou próximas do vencimento.':'Não há valor contratual em risco identificado no filtro atual.',
      action:exposure>0?'Priorizar a tratativa das solicitações de maior valor primeiro.':'Manter o monitoramento de prazos e valores.'}
  ];

  cards.sort((a,b)=>toneRank(a.tone)-toneRank(b.tone));

  return {
    total,healthScore,headlineTone,headlineIcon,headline,headlineText,
    criticalCount:cards.filter(c=>c.tone==='danger').length,
    warningCount:cards.filter(c=>c.tone==='warning').length,
    exposure,cards
  };
}
function renderContractInsights(list){
  const el=document.getElementById('contractInsightsSection');
  if(!el)return;
  const analysis=buildContractInsights(list);
  const cardsHtml=analysis.cards.map(card=>\`
    <article class="contract-insight-card contract-insight-card--\${card.tone}">
      <div class="contract-insight-card-head">
        <span class="contract-insight-card-icon">\${card.icon}</span>
        <div>
          <small>\${esc(card.eyebrow)}</small>
          <strong>\${esc(card.title)}</strong>
        </div>
        <em class="contract-insight-priority contract-insight-priority--\${card.tone}">\${toneLabel(card.tone)}</em>
      </div>
      <div class="contract-insight-card-value">
        <strong>\${esc(card.value)}</strong>
        <span>\${esc(card.meta)}</span>
      </div>
      <p>\${esc(card.description)}</p>
      <footer><b>Ação:</b> \${esc(card.action)}</footer>
    </article>
  \`).join('');

  el.innerHTML=\`
    <div class="contract-insight-title">
      <span>⚠️</span>
      <div>
        <h3>Alertas de contratos</h3>
        <p>Análises críticas recalculadas automaticamente conforme o Kanban e os filtros do dashboard.</p>
      </div>
    </div>
    <div class="contract-insight-summary">
      <article class="contract-insight-summary-card contract-insight-summary-card--wide contract-insight-summary-card--\${analysis.headlineTone}">
        <span class="contract-insight-summary-icon">\${analysis.headlineIcon}</span>
        <div>
          <h4>\${esc(analysis.headline)}</h4>
          <p>\${esc(analysis.headlineText)}</p>
          <time>Recalculado automaticamente em \${esc(new Date().toLocaleString('pt-BR'))}</time>
        </div>
      </article>
      <article class="contract-insight-summary-card contract-insight-summary-card--\${analysis.healthScore>=80?'success':analysis.healthScore>=58?'warning':'danger'}">
        <small>Índice de controle</small>
        <strong>\${analysis.healthScore}/100</strong>
        <p>Composto por prazo, qualidade das solicitações, fluxo e concentração operacional.</p>
      </article>
      <article class="contract-insight-summary-card contract-insight-summary-card--\${analysis.criticalCount?'danger':analysis.warningCount?'warning':'success'}">
        <small>Análises críticas</small>
        <strong>\${analysis.criticalCount}</strong>
        <p>\${analysis.warningCount} análise(s) adicional(is) em atenção.</p>
      </article>
      <article class="contract-insight-summary-card contract-insight-summary-card--\${analysis.exposure>0?'warning':'success'}">
        <small>Exposição financeira</small>
        <strong>\${esc(currencyFmt(analysis.exposure))}</strong>
        <p>Valor de solicitações vencidas ou próximas do vencimento.</p>
      </article>
    </div>
    <div class="contract-insight-grid-head">
      <h4>Análises automáticas priorizadas</h4>
      <span>\${analysis.total} solicitação(ões) consideradas no filtro atual</span>
    </div>
    <div class="contract-insight-grid">\${cardsHtml || '<div class="empty-col">Sem solicitações no filtro atual.</div>'}</div>
  \`;
}
...
function openRequestDetails(id){
  ...
  const traffic=contractTrafficInfo(r);
  const requestHasError=contractRequestHasError(r);
  const valueRaw=contractValue(r);
  const valueLabel=valueRaw?currencyFmt(valueRaw):'Não informado';
  const comments=getContractComments(r);
  const responsibleObservation=String(r.observacoesResponsavel||r.data?.observacoesResponsavel||'').slice(0,240);
  title.textContent=\`\${contractDisplayTag(r)} - \${requestTitle(r)}\`;
  subtitle.textContent=\`\${r.tipo||'Solicitação'} • \${r.status||''}\`;
  body.innerHTML=\`
    <div class="detail-edit-actions"><button type="button" onclick="startRequestEdit('\${esc(r.id)}')">\${contractIcon('pencil',14)} Editar informações</button></div>

    <div class="detail-summary">
      <div class="detail-summary-tile detail-summary-tile--\${traffic.key}"><span>Semáforo</span><strong>\${esc(traffic.label)}</strong></div>
      <div class="detail-summary-tile"><span>Fase atual</span><strong>\${esc(r.status||'')}</strong></div>
      <div class="detail-summary-tile"><span>SLA total</span><strong>\${fmtDuration(totalSlaMs(r))}</strong></div>
      <div class="detail-summary-tile"><span>Tipo</span><strong>\${esc(contractKindLabel(r))}</strong></div>
      <div class="detail-summary-tile"><span>Valor</span><strong>\${esc(valueLabel)}</strong></div>
      <div class="detail-summary-tile detail-summary-tile--\${requestHasError?'red':'green'}"><span>Situação</span><strong>\${requestHasError?'Solicitação com erro':'Sem erro'}</strong></div>
    </div>

    <div class="contract-card-observation">
      <label for="contractObservation_\${esc(r.id)}">Observações do responsável</label>
      <textarea id="contractObservation_\${esc(r.id)}" maxlength="240" placeholder="Registre orientações, pendências ou informações importantes..." oninput="updateContractObservationCounter(this);queueContractObservationSave('\${esc(r.id)}',this)" onblur="persistContractResponsibleObservation('\${esc(r.id)}',this,true)">\${esc(responsibleObservation)}</textarea>
    ...
    <div class="detail-comments-panel">
      <h3>Comentários e alertas (\${comments.length})</h3>
      \${comments.length ? \`<div class="detail-comment-list">\${comments.map(comment=>\`
        <div class="detail-comment">
          <div class="detail-comment-head"><strong>\${esc(comment.authorName||'Equipe de contratos')}</strong><span>\${comment.at?new Date(comment.at).toLocaleString('pt-BR',{dateStyle:'short',timeStyle:'short'}):''}</span></div>
          <p>\${esc(comment.text||'')}</p>
        </div>
      \`).join('')}</div>\` : '<div class="detail-comment-empty">Nenhum comentário registrado para esta solicitação.</div>'}
      <div class="detail-comment-form">
        <textarea id="contractCommentInput" placeholder="Registre uma orientação, pendência ou alerta sobre esta solicitação..."></textarea>
        <button class="secondary" type="button" onclick="submitContractComment('\${esc(r.id)}')">\${contractIcon('message',14)} Adicionar comentário</button>
      </div>
    </div>
  \`;
  ...
  el.innerHTML=\`<section class="process-block">
    ...
    <div class="kanban unified-kanban">
      \${groupedStages.map(({group,groupIndex,stage:st,stageIndex})=>{
        ...
        return \`<div class="col unified-col \${processClass}\${separatorClass}" ondragover="event.preventDefault()" ondrop="handleContractKanbanDrop(event,'\${esc(st)}')">
          ...
          <div class="kanban-card-stack">\${items.length ? items.map(r=>{
            ...
            const valueRaw=contractValue(r);
            const valueLabel=valueRaw?currencyFmt(valueRaw):'';
            const stageTone=CONTRACT_STAGE_TONE[st]||'em-andamento';
            const createdLabel=r.createdAt?new Date(r.createdAt).toLocaleString('pt-BR',{dateStyle:'short',timeStyle:'short'}):'Não informado';
            const attachmentCount=contractAttachmentCount(r);
            return \`<div class="card contract-freight-card \${trafficKey}\${requestHasError?' request-error':''}" draggable="true" ondragstart="event.dataTransfer.setData('text/plain','\${esc(r.id)}')" onclick="openRequestDetails('\${r.id}')">
              <div class="contract-compact-top">
                <div class="contract-compact-title-wrap">
                  <div class="contract-compact-id">\${esc(contractDisplayTag(r))}</div>
                  <div class="contract-compact-title">\${esc(title)}</div>
                </div>
                <span class="contract-status-chip contract-status-chip--\${stageTone}">\${esc(st)}</span>
              </div>

              <div class="contract-compact-lines">
                <div><span class="contract-line-icon">\${contractIcon('clipboard',11)}</span><span><b>Solicitante:</b> \${esc(r.solicitante||'Não informado')}</span></div>
                <div><span class="contract-line-icon">\${contractIcon('building',11)}</span><span><b>Obra / centro de custo:</b> \${esc(centerLabel)}</span></div>
                <div><span class="contract-line-icon">\${contractIcon('calendar',11)}</span><span><b>Prazo atendimento:</b> \${esc(dueText)}</span></div>
                <div><span class="contract-line-icon">\${contractIcon('calendar',11)}</span><span><b>Criado em:</b> \${esc(createdLabel)}</span></div>
              </div>

              <div class="contract-route-pill">
                <span>\${esc(kind)}</span>
                <span>\${esc(processLabel)}</span>
              ...
              <div class="contract-compact-badges">
                \${valueLabel?\`<span class="contract-value-pill">\${esc(valueLabel)}</span>\`:''}
                <span class="contract-mini-badge traffic-\${trafficKey}">\${esc(trafficLabel)}</span>
                \${attachmentCount?\`<span class="contract-attachment-count">\${contractIcon('paperclip',11)} \${attachmentCount}</span>\`:''}
                <span class="contract-mini-badge request-error-badge \${requestHasError?'show':''}">SOLICITAÇÃO COM ERRO</span>
              </div>

              <div class="contract-deadline-strip contract-deadline-strip--\${trafficKey}">
                <span>Prazo: \${esc(dueText)}</span>
                <strong>\${esc(due.text)}</strong>
              </div>
              ...
              <div class="contract-compact-timer">
                ...
                <div><span>Acumulado da fase</span><strong data-contract-total-timer="\${esc(r.id)}">\${totalTime}</strong></div>
              ...
              <div class="contract-phase-selector-wrap" onclick="event.stopPropagation()">
                <label>Fase da solicitação</label>
              ...
              <label class="contract-error-toggle" onclick="event.stopPropagation()">
                <input type="checkbox" \${requestHasError?'checked':''} onclick="event.stopPropagation()" onchange="event.stopPropagation();toggleContractRequestError('\${r.id}',this.checked,this)">
                <span class="contract-error-toggle-icon">⚠️</span>
              ...
              <div class="contract-compact-actions" onclick="event.stopPropagation()">
                ...
                <button class="contract-delete-btn secondary" type="button" onclick="event.stopPropagation();deleteContractRequest('\${r.id}')">Excluir</button>
              </div>
            </div>\`;
...
function deleteContractRequest(id){
  if(!confirm('Excluir esta solicitação de contrato? Essa ação não pode ser desfeita.'))return;
...
const CONTRACT_STAGE_TONE={
  'Não Iniciado':'nao-iniciado',
  'Em Elaboração':'nao-iniciado',
  'Aguardando Validação':'em-andamento',
  'Enviado para Assinatura':'em-andamento',
  'Contrato Assinado':'finalizado',
  'Em Cadastro no Compor':'em-andamento',
  'Aprovado no Compor':'finalizado'
};
function contractAttachmentCount(r){
  const data=r.data||{};
  return Object.keys(data).filter(k=>/anexo/i.test(k)&&data[k]!==undefined&&data[k]!==null&&String(data[k]).trim()!=='').length;
}
function getContractComments(r){
  const raw=r?.comentarios;
  return Array.isArray(raw)?raw:[];
}
function appendContractComment(id,text){
  ...
  const comment={
    ...
    text:trimmed,
    authorName:identity.name||identity.email||'Equipe de contratos'
  };
  let changed=false;
  requests=requests.map(request=>{
    if(String(request.id)!==String(id))return request;
    changed=true;
    return {...request,comentarios:[...getContractComments(request),comment]};
  });
  if(changed)saveRequests();
  return changed;
}
function submitContractComment(id){
  const textarea=document.getElementById('contractCommentInput');
  const text=textarea?textarea.value:'';
  if(!String(text||'').trim()){toast('Escreva um comentário antes de enviar.');return;}
  const ok=appendContractComment(id,text);
  if(!ok)return;
  renderKanban();
  renderDashboard();
...
function contractTrafficInfo(r){
  const due=deadlineInfo(r);
  const completed=r.status==='Aprovado no Compor';
  const key=completed?'gray':(due.cls==='orange'?'yellow':due.cls==='blue'?'gray':due.cls);
  const label=completed?'Finalizado':(due.status==='Em atraso'?'Atrasado':due.status==='Em alerta'?'Em alerta':due.status==='No prazo'?'No prazo':'Sem prazo');
  return {key,label,due};
}
function contractDisplayTag(r){
  const centro=String(r.centro||r.data?.centro_obra||r.data?.centro_departamento||'').trim()||'SC';
  const raw=String(r.dataSolicitacao||r.data?.data_solicitacao||r.createdAt||'').trim();
  let mm='--',yyyy='----';
  ...
  return \`CT-\${centro}-\${mm}/\${yyyy}\`;
}
function handleContractKanbanDrop(event,newStage){
  event.preventDefault();
  const id=event.dataTransfer.getData('text/plain');
  if(id)setReqStage(id,newStage);
}
