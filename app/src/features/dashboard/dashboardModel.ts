...
import type { EmbeddedStorageSnapshot } from "../../services/embeddedSync";
import {
...
export type DashboardFamily = "operacao" | "ativo" | "base";
export type KpiTone = "neutral" | "success" | "warning" | "danger" | "blue";
...
export type CriticalTone = "success" | "monitor" | "warning" | "danger";
...
export const familyLabels: Record<DashboardFamily, string> = {
  operacao: "Processo de atendimento",
  ativo: "Ativos e contratos",
  base: "Base cadastral",
};

export function buildDashboardRows(data: DashboardSourceData): DashboardRow[] {
...
export function buildCriticalInsights(rows: DashboardRow[]): CriticalInsight[] {
  ...
  rows.forEach((row) => {
    ...
    insights.push({
      ...
      action:
        donePercent < 35 && row.family === "operacao"
          ? "Revisar gargalos do Kanban e redistribuir demandas abertas."
          : "Manter revisao periodica para preservar o indice.",
  ...
  if (baseRows.length) {
    ...
    insights.push({
      ...
      description: baseRisk
        ? "Cadastros sem contato, inativos ou incompletos reduzem a velocidade de acionamento."
        : "As bases cadastrais estao em nivel adequado para consulta.",
      action: baseRisk
        ? "Completar e-mails, telefones, status e classificacoes antes de novas importacoes."
        : "Manter validacoes na entrada e edicao dos cadastros.",
  ...
  if (assetRows.length) {
    ...
    insights.push({
      ...
      description: assetRisk
        ? "Contratos, multas ou manutencoes pedem acompanhamento para evitar ruptura operacional."
        : "Nao ha exposicoes relevantes nos ativos filtrados.",
  ...
  return insights.sort((a, b) => b.priority - a.priority || toneRank(b.tone) - toneRank(a.tone)).slice(0, 12);
}

export function buildControlledInsight(rows: DashboardRow[]): CriticalInsight {
  const totals = rowTotals(rows);
  return {
...
export function toneRank(tone: CriticalTone) {
...
function summarizeFretes(rows: Array<Record<string, unknown>>): DashboardRow {
  ...
  const vencidos = rows.filter(
    (item) =>
      !isFinalStatus(getText(item, ["status", "situacao", "fase"])) &&
      isPastDate(firstDefined(item, ["dataLimiteEntrega", "prazoEntrega", "dataEntrega", "dataColetaMaterial"]))
  ).length;
...
function summarizeFrota(vehicles: Array<Record<string, unknown>>, fines: Array<Record<string, unknown>>): DashboardRow {
  ...
  const contratosVencendo = vehicles.filter((item) =>
    isWithinDays(firstDefined(item, ["terminoContrato", "fimContrato", "dataFimContrato", "vencimentoContrato"]), 45)
  ).length;
...
function summarizeEstoque(pedidos: Array<Record<string, unknown>>, itens: Array<Record<string, unknown>>): DashboardRow {
  ...
  const abaixoMinimo = itens.filter(
    (item) => getNumber(item, ["qty", "quantidade", "saldo", "estoque"]) <= getNumber(item, ["min", "minimo", "estoqueMinimo"])
  ).length;
...
export function sumRows(rows: DashboardRow[], key: "demanda" | "emAberto" | "finalizados" | "riscoSla") {
  return rows.reduce((acc, row) => acc + row[key], 0);
}

export function rowTotals(rows: DashboardRow[]) {
  return rows.reduce(
    (acc, row) => ({
